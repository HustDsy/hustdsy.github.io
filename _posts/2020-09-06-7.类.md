...
另外这里要区分的点是==const int function()==是代表返回值为常量，而非常量成员函数
...
![image-20200903110214595](/Users/dongshenyu/Library/Application Support/typora-user-images/image-20200903110214595.png)
...
Tip5:友元函数再探，比如类A和B，B的函数需要访问A，在A中声明友元类B，如果只想要B的某一个函数访问A，那么在A中声明这个函数		 就行。友元函数不具有传递性，B的友元函数无法访问A。
